...
syn keyword octaveSetVarFun contained  EDITOR EXEC_PATH I IMAGE_PATH Int J NA
...
syn keyword octaveSetVarFun contained  optimize_subasgn_calls
...
syn keyword octaveVariable contained  optimize_subasgn_calls
...
syn keyword octaveFunction contained  __recorder_stop__ __recorder_drawnow__
...
syn keyword octaveFunction contained  annotation anova any arch_fir arch_rnd
...
syn keyword octaveFunction contained  doc_cache_creates dos dot double drawnow
...
syn keyword octaveFunction contained  geoinv geopdf geoinv get
...
syn keyword octaveFunction contained  jet khbit kendall keyboard kill
...
syn keyword octaveFunction contained  ranks rat rats rcond rdivide raddir
...
syn keyword octaveFunction contained  rectangle rectint recycle reducepath
...
syn keyword octaveFunction contained  sec secd semicolon semilogx
...
syn keyword octaveFunction contained  sleep slice slice3 sombrero sort
...
syn keyword octaveFunction contained  spones spparams sprand sprandn sprandsym
...
syn keyword octaveFunction contained  stdnormal_rand stem stem3 stemleaf stft
...
" (highlight user-defined functions within script)
function! s:CheckForFunctions()
  let i = 1
  while i <= line('$')
    let line = getline('i')
    " Only look for functions at start of line.
    " Commented function (i.e., '# function ...' will not trigger as match returns 3
    if match(line, '\Cfunction') == 0
      ...
      " Include anonymous functions i.e.,  'func = @(...)'.
      " Use 'contained' keyword to prevent highlighting on LHS of the '=' operator
...
" Don't highlight Octave keywords on LHS of '=' operator - these are user-defined
syn match octaveUserVar  "\<\h\w*\ze[^<>!~="']\{-}==\@!"
syn match octaveUserVar  "\<\h\w*\s*[<>!~=]=" contains=octaveVariable

""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
" Errors (placed early so they may be overridden by more specific rules
" Struct with nonvalid identifier that starts with a number (e.g., 1.a or a.1b)
syn region octaveError  start="\<\d\+\(\w*\.\)\@="  end="[^0-9]"he=s-1 oneline
syn region octaveError  start="\.\d\+\(\w*\)\@="hs=s+1  end="[^0-9]"he=s-1 oneline
" Numbers with double decimal points (e.g., 1.2.3)
syn region octaveError  start="\<-\?\d\+\.\d\+\.[^*/\\^]"hs=e-1 end="\>"  oneline
syn region octaveError  start="\<-\?\d\+\.\d\+[eEdD][-+]\?\d\+\.[^*/\\^]"hs=e-1 end="\>"  oneline

" Operators
" Uncomment the "Hilink octaveOperator" command at bottom of file to highlight these
...
" User Variables
" Uncomment the following line as well as the "Hilink octaveIdentifier" command at
" bottom of file to highlight these
...
" Floating point number with decimal point and (optionally) an exponent
syn match octaveFloat   "\<\d\+\(\.\d*\)\?\([edED][-+]\?\d\+\)\?[ij]\?\>"
" Floating point number that starts with a decimal point and (optionally) contains an exponent
...
" Delimiters and 'transpose' operator
syn match octaveDelimiter          "[][(){}@]"
syn match octaveTransposeOperator  "[])}[:alnum:]._]\@<='"

" Tabs (in case they should be highlighted as errors)
syn match octaveTab  "\t"
...
" Comments (oder of matches *IS* important here)
...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
" Apply highlight groups to the syntax groups defined above
