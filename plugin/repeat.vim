...
" Installation:
" Place in either ~/.vim/plugin/repeat.vim (to load at startup) or
...
" Special function to avoid spurious repeats in related, naturally repeating
...
function! repeat#wrap(command,count)
    ...
    execute 'normal! '.(a:count ? a:count : '').a:command . (&foldopen =~# 'undo\|all' ? 'zv' : '')
    if preserve
        let g:repeat_tick = b:changedtick
    endif
endfunction
