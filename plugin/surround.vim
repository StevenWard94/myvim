...
function! s:fixindent(str, spc)
  ...
  if ! &expandtab
    let str = substitute(str, '\s\{'.&tabstop.'\}', "\t", 'g')
  endif
  return str
endfunction
...
function! s:dosurround(...)  "{{{1
  ...
  if char == '/'
    execute 'normal! '.strcount.'[/d'.strcount.']/'
  elseif char =~# '[[:punct:][:space:]]' && char !~# '[][(){}<>"''`]'
    ...
    execute 'normal! d'.strcount.'i'.char
  endif
  ...
  elseif char =~# '[[:punct:][:space:]]' && char !~# '[][(){}<>]'
    ...
    call search('\m.', 'bW')
    execute "normal! da".char
  endif

  let removed = getreg('"')
  let rem2 = substitute(removed, '\n.*', '', '')
  let oldhead = strpart(oldline, 0, strlen(oldline) - strline(rem2))
  let oldtail = strpart(oldline,    strlen(oldline) - strline(rem2))
