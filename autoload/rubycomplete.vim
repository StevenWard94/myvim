" vim: set tw=120 fo=c1jb wm=0 sw=4 ts=4 sts=4 foldenable foldlevel=0 foldmarker=\\begin,\\end foldmethod=marker:
" ----------------------------------------------------------------------------------------------------------------------
"
...
" \begin ruby-side code
function! s:DefRuby()
    ruby << RUBYEOF
    # \begin ruby completion

    begin
        require 'rubygems'      # assumed to be safe...
    rescue Exception
        #ignore?
    end
    ...
    # \begin buffer analysis magic
      ...
      def load_buffer_class(name)
        ...
        load_buffer_class( $2 ) if pare != nil && $2 != name    # if needed, load parent class

        mixre = /.*\n\s*(include|prepend)\s*(.*)\s*\n/.match( classdef )
        load_buffer_module( $2 ) if mixre != nil && $2 != name  # if needed, load mixins
      ...
      def get_buffer_entity(name, vimfun)
        loading_allowed = VIM::evaluate("exists('g:rubycomplete_buffer_loading') && rubycomplete_buffer_loading")
    ...
    # \end buffer analysis magic

    # \begin main completion code
