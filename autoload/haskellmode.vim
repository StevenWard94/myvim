...
function! haskellmode#GetNameSymbol(line, col, off)
  ...
  let symbolic  = fullid[-1:-1] =~ symbol     " could also be incomplete qualified id ending w/ '.'
...
function! haskellmode#GatherImports()
  ...
  while i <= line('$')
    ...
    if !empty(res)
      ...
      let listPat   = '\(\s*\((.*)\)\)\?'
      let importPat = prefixPat . modulePat . asPat . hidingPat . listPat
...
function! haskellmode#ListIntersect(list1, list2)
  let l = []
  for e in a:list1
    if index(a:list2,e) != -1 | let l += [e] | endif
  endfor
  return l
endfunction
...
function! haskellmode#ListUnion(list1, list2)
  let l = []
  for e in a:list2
    if index(a:list1,e) == -1 | let l += [e] | endif
  endfor
  return a:list1 + l
endfunction
...
" return number of last line as well as the collected import statement
" (assumes opening parenthesis, if any, is on the first line)
