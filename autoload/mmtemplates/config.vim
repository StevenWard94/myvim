...
" need compatible
if &compatible || ( exists('g:TemplatesConfig_Version') && g:TemplatesConfig_Version != 'searching' && !exists('g:TemplatesConfig_DevelopmentOverwrite') )
    finish
endif
...
elseif exists('g:TemplatesConfig_VersionSearch')
    ...
    "--------------------------------------------------------------------------------
    " s:VersionComp : Compare two version numbers.   \begin3
    "
    " Parameters:
    ...
    function! s:VersionComp ( op1, op2 )
        ...
        for i in range( 0, max( [ len( l1 ), len( l2 ) ] ) - 1 )
            " until now, all fields were equal
            if len ( l2 ) <= i
...
"-------------------------------------------------------------------------------
"  === Script: Auxiliary Functions ===   \begin1
"-------------------------------------------------------------------------------

"-------------------------------------------------------------------------------
" s:UserInput : Input using a highlighting prompt.   \begin2
"
" Parameters:
...
function! s:UserInput ( prompt, text, ... )
    ...
    call inputrestore()
    echohl None
    let retval = substitute( retval, '^\s\+', '', '' )       " remove leading whitespace
    let retval = substitute( retval, '\s\+$', '', '' )       " remove trailing whitespace
    return retval
