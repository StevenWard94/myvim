" repeat.vim - Let the 'repeat' command repeat plugin mappings
" Author:              Tim Pope <http://tpo.pe/> 
" Version:             1.2
" Last [local] Change: 2018 Feb 19
"
" GetLatestVimScripts: 2136 1 :AutoInstall: repeat.vim

" Installation:
" Place 'repeat.vim' in in either ~/.vim/plugin (to always load at start up) or
"     ~/.vim/autoload (to load automatically as functions are needed)
"
" License:
    ...
    try
        if g:repeat_tick == b:changedtick
            ...
            if g:repeat_reg[0] ==# g:repeat_sequence && !empty(g:repeat_reg[1])
                if g:repeat_reg[1] ==# '='
                    " This causes the expression to be re-evaluated upon repeat,
            ...
            let g = g:repeat_count
            let s = g:repeat_sequence
            let cnt = c == -1 ? "" : ( a:count ? a:count : (c ? c : '') )

            if (( v:version == 703 && has('patch100') ) || ( v:version == 704 && !has('patch601') ))
                execute 'normal ' . r . cnt . s
            else
                call feedkeys(s, 'i')
